...
%alterar aqui o codigo
mini = 6*x1.^2-4*x1.*x2-2*x1+1+2*x2.^2;
x22 = (3.5-7*X)/4;

colormap 'jet'

contour(x1,x2,mini,100,"--")
hold on
text(0.2,0.2,'(0.25,0.25)')
text(0.24,0.9,'$1-x_1-x_2$','Interpreter','latex')
text(0.54,0.08,'$7x_1+4x_2+1$','Interpreter','latex')

%restrições

hatchedline(X,x22,"r",45);
hatchedline(X,1-X,"k",45);
...
%encontra o minimo da função, centros das elipses
index_position=find(mini==min(min(mini)))
x1(index_position)
x2(index_position)


scatter( x1(index_position), x2(index_position),"kx")

%metodo que tirei da net para fazer as linha com riscas
%metodo hatched lines
